...
static int is_a_listener(lserver_t *server, lclient_t *ptr)
{
	...
	lvector_emplace_back(server->clients, lclient_create, server->config.client_buffer_size,
			     NULL, 0);
	...
	if (server->config.cbs.on_connect_callback)
		server->config.cbs.on_connect_callback(server, new, ptr->socket.port,
						       server->config.cbs.on_connect_userdata);
	return (0);
}
	...
	lvector_foreach(evt, server->revents)
	{
		...
		if (rd == 0) {
			...
			if (server->config.cbs.on_disconnect_callback != NULL)
				server->config.cbs.on_disconnect_callback(
					server, ptr, server->config.cbs.on_disconnect_userdata);
